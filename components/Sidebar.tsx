    ...
    <>
      ...
      <aside
        className={`
          ...
          border-l border-white/10 bg-[#0A0A0A]/95
      ...
      >
        <div className="flex flex-col h-full w-[280px] md:w-[320px] p-4 md:p-5 transition-opacity duration-300">
          <div className="flex justify-between items-center mb-5 pb-4 border-b border-white/10">
            <span className="text-[#EAEAEA] font-semibold text-lg">المحادثات</span>
            <div className="flex gap-1.5 items-center">
               {onToggleFocus && (
                 <button onClick={onToggleFocus} className="hidden md:flex text-gray-500 hover:text-[#D4AF37] transition-all p-1.5 rounded-lg border border-transparent hover:border-[#D4AF37]/20 hover:bg-[#D4AF37]/10" aria-label="وضع التركيز للقراءة" title="إخفاء القائمة الجانبية (وضع التركيز)">
               ...
               <button onClick={onClose} className="md:hidden text-gray-400 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-white/5 border border-transparent" aria-label="إغلاق القائمة">
          ...
          <button
            onClick={onNewChat}
            className="w-full py-2.5 px-4 mb-5 rounded-lg border border-white/10 text-white bg-white/5 hover:bg-white/10 transition-colors duration-200 flex items-center justify-center gap-2 font-medium"
          ...
          <div className="mb-3 flex items-center justify-between px-1">
            ...
            <p className="text-xs text-gray-400">{chats.length} محادثة</p>
          </div>

          <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
            {chats.length === 0 ? (
              <div className="text-center text-gray-500 text-sm py-8 border border-dashed border-white/10 rounded-2xl">لا توجد محادثات بعد</div>
            ) : (
              chats.map(chat => {
                ...
                return (
                  <div
                    ...
                    className={`group relative cursor-pointer rounded-lg px-3 py-2.5 border transition-all duration-200 flex items-center gap-2
                      ${
                        isActive
                          ? 'border-[#D4AF37]/40 bg-gradient-to-r from-[#D4AF37]/20 to-[#D4AF37]/5 text-[#E8D499] shadow-[0_0_15px_rgba(212,175,55,0.2)]'
          ...
          <div className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-2 relative">
            ...
            <p className="text-gray-600 text-[11px] text-center mt-2.5">© 2025 Salaf AI</p>
          </div>
        </div>
      </aside>
    </>
